  ...
  getDutInfo() {
    this.candidatParcours.getDiplomes().then((res) => {
      ...
      if (index !== -1) {
        ...
        this.candidatDutForm
          .get('intitule')
          ?.setValue(this.candidatDut?.intitule);
        this.candidatDutForm.get('type')?.setValue(this.candidatDut?.type);
        this.candidatDutForm
          .get('mention')
          ?.setValue(this.candidatDut?.mention);
        this.candidatDutForm
          .get('moyen_generale')
          ?.setValue(this.candidatDut?.moyen_generale);
        this.candidatDutForm.get('pays')?.setValue(this.candidatDut?.pays);
        this.candidatDutForm
          .get('dateCommission')
          ?.setValue(this.candidatDut?.dateCommission);
        this.candidatDutForm
          .get('etablissement')
          ?.setValue(this.candidatDut?.etablissement);
        this.candidatDutForm
          .get('specialite')
          ?.setValue(this.candidatDut?.specialite);
        this.candidatDutForm
          .get('province')
          ?.setValue(this.candidatDut?.province);
